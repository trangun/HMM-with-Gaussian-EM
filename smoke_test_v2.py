...
def get_output(filename):
    ...
    if filename.endswith('_hmm_gaussian.py'):
        cmd = 'python {} --nodev --iterations 1 --clusters_file gaussian_hmm_smoketest_clusters.txt --data_file {} --print_params'.format(filename, os.path.join(DATA_DIR,'points.dat'))
    else:
        cmd = 'python {} --nodev --iterations 1 --clusters_file aspect_hmm_smoketest_clusters.txt --data_file {} --print_params'.format(filename, os.path.join(DATA_DIR,'pairs.dat'))
...
def main():
    ...
    if filenames[0].endswith('_hmm_gaussian.py'):
        ...
        reqs = [
            ...
            Req('Train LL: -4.495267972400826', 'Training average log-likelihood'),
            Req('Initials: 0.08411545977900343 | 0.9158845402209965','Initials'),
            Req('Transitions: 0.5974194744896205 0.40258052551037954 | 0.4038490792315056 0.5961509207684944','Transitions'),
